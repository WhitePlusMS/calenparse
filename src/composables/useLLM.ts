...
核心原则：
- 仔细识别文本中的所有时间节点和阶段
- 如果文本描述了同一活动的多个阶段（如报名、提交、评审、公示、决赛等），请为每个阶段创建独立的事件
- 每个事件的标题应该包含活动名称和阶段名称（格式：活动名-阶段名，例如："高德大赛-线上报名+提交作品"）
- 如果文本中只有一个独立事件，则只返回一个事件对象
- 如果文本中有多个完全独立的不同活动，则为每个活动创建独立的事件

要求：
1. 提取以下字段（如果存在）：
   - title: 事件标题（格式：活动名-阶段名，如果只有一个阶段则只写活动名）
   - startTime: 开始时间（ISO 8601格式，例如：2024-11-22T13:00:00）
   - endTime: 结束时间（ISO 8601格式）
   - isAllDay: 是否全天事件（布尔值）
   - location: 地点
   - description: 描述或备注（包含该阶段的详细信息和要求）
...
2. 时间识别规则：
   ...
   - 对于单个日期（如"2026年1月9日"），如果文本中没有明确开始和结束时间，startTime 和 endTime 都设置为该日期，isAllDay 为 true
...
3. 多阶段活动处理规则：
   - 识别活动的主名称（如"高德大赛"）
   - 识别每个阶段的名称和时间范围（如"线上报名+提交作品"、"初赛评审"、"初赛结果公示"等）
   - 为每个阶段创建独立的事件对象，标题格式为"活动名-阶段名"
   - 每个阶段事件的 description 可以包含该阶段的具体要求和说明
